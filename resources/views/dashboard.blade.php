...
<div class="dashboard-container">
  ...
  <div class="cards-row">
    <a class="dash-card" href="{{ route('admin.comprobantes.index', ['tipo' => 'mensual', 'estado' => 'pendiente']) }}">
      ...
      <div class="dash-card__count">{{ $pendientesComprobantes ?? 0 }} pendientes</div>
    </a>
    <a class="dash-card" href="{{ url('/admin/horas?estado=pendiente') }}">
      <div class="dash-card__title">Horas de trabajo</div>
      <div class="dash-card__count">{{ $pendientesHoras ?? 0 }} pendientes</div>
    </a>
    <a class="dash-card" href="{{ url('/admin/exoneraciones?estado=pendiente') }}">
      <div class="dash-card__title">Exoneraciones</div>
      <div class="dash-card__count">{{ $pendientesExoneraciones ?? 0 }} pendientes</div>
    </a>
    <a class="dash-card" href="{{ route('admin.solicitudes.index', ['estado' => 'pendiente']) }}">
      <div class="dash-card__title">Nuevos socios</div>
      <div class="dash-card__count">{{ $pendientesSocios ?? 0 }} pendientes</div>
    </a>
  </div>
  ...
  {{-- Módulo de gastos: opcional, se implementará en entregas posteriores --}}
  <div class="module">
    <div class="module__title">Módulo de gastos (futuro)</div>
